...
from yapf_third_party._ylib2to3.pygram import python_symbols
...
class FormatTokena(object):
  ...
  def __init__(self, node, name):
    ...
    #                                         pytree_utils.Annotation.SUBTYPE)
    self.subtypes = {subtypes.NONE}  # if not stypes else stypes
    self.is_pseudo = False  #hasattr(node, 'is_pseudo') and node.is_pseudo

  @property
  def type(self):
    if isinstance(self.node.wrapped, cst.Import):
      return python_symbols.import_stmt
    elif isinstance(self.node.wrapped, cst.ClassDef):
      return python_symbols.classdef
    elif isinstance(self.node.wrapped, cst.FunctionDef):
      return python_symbols.funcdef
    elif isinstance(self.node.wrapped, cst.Comma):
      return token.COMMA
    elif isinstance(self.node.wrapped, cst.Integer):
      return token.NUMBER
    elif isinstance(self.node.wrapped, cst.Name):
      return token.NAME
    elif isinstance(self.node.wrapped, cst.Return):
      return python_symbols.return_stmt
    elif isinstance(self.node.wrapped, cst.SimpleString):
      return token.STRING
    else:
      raise ValueError(f'no type for {type(self.node.wrapped)}')
  ...
  def value(self):
    if isinstance(self.node.wrapped, cst.Import):
      ...
      return 'return'
    elif hasattr(self.node.wrapped, 'value'):
      print(
          f'for {type(self.node.wrapped)} ret value: {self.node.wrapped.value}')
      return self.node.wrapped.value
    else:
      raise ValueError(f'no value for {type(self.node.wrapped)}')
