...
def UnwrapModule(module: cst.Module) -> list[logical_line.LogicalLine]:
  ...
  return visitor.GetLogicalLines()
  # llines = unwrapper.GetLogicalLines()
  # llines.sort(key=lambda x: x.lineno)
  # return llines


class ModuleVisitor(cst.CSTTransformer):
  ...
  def on_visit(self, node: cst.CSTNode) -> bool:
    ...
    if isinstance(node, cst.Module):
      imp = node.body[0].body[0]
      imp_name = imp.names[0].name
      pass
    elif isinstance(node, cst.SimpleStatementLine):
      pass
    elif isinstance(node, cst.SimpleWhitespace):
      pass
    elif isinstance(node, cst.TrailingWhitespace):
      pass
    elif isinstance(node, cst.Newline):
      self._StartNewLine()
    elif isinstance(node, cst.EmptyLine):
      pass
    elif isinstance(node, cst.Comment):
      pass
      # self._cur_logical_line.AppendToken(
      #     format_token.FormatToken(node, 'comment'))
    elif isinstance(node, cst.Import):
      self._cur_logical_line.AppendToken(
          format_token.FormatToken(node, 'import'))
    elif isinstance(node, cst.ClassDef):
      self._cur_logical_line.AppendToken(
          format_token.FormatToken(node, 'classdef'))
    elif isinstance(node, cst.IndentedBlock):
      self._cur_depth += 1
    elif isinstance(node, cst.FunctionDef):
      self._cur_logical_line.AppendToken(
          format_token.FormatToken(node, 'functiondef'))
      ...
      pass
    elif isinstance(node, cst.Comma):
      self._cur_logical_line.AppendToken(
          format_token.FormatToken(node, 'comma'))
    elif isinstance(node, cst.Return):
      self._cur_logical_line.AppendToken(
          format_token.FormatToken(node, 'return'))
    elif isinstance(node, cst.SimpleString):
      self._cur_logical_line.AppendToken(
          format_token.FormatToken(node, 'simplestring'))
    elif isinstance(node, cst.Integer):
      self._cur_logical_line.AppendToken(
          format_token.FormatToken(node, 'integer'))
    elif isinstance(node, cst.Name):
      self._cur_logical_line.AppendToken(format_token.FormatToken(node, 'name'))
    else:
      raise ValueError(f'unknown node type {type(node)}')

    # if isinstance(node, cst.Import):
    # self._cur_logical_line.AppendToken(
    #     format_token.FormatToken(node, 'node_name'))
    return True
  ...
  def on_leave_attribute(self, original_node: cst.CSTNode,
                         attribute: str) -> None:
    pass
...
#     self._cur_depth = 0
  ...
  def GetLogicalLines(self) -> list[logical_line.LogicalLine]:
